...
原理：
  调用 IHG 官网搜索页同款公开接口
  POST {domain}/availability/v3/hotels/offers?fieldset#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
IHG 积分房（Reward Night）监控脚本 —— 配合 GitHub Actions 定时运行。
...
通知规则（简单直接）:
  - 查到有积分房 → 发邮件提醒（每 15 分钟一封，直到订上或没房）
  - 没有积分房 → 不发邮件
  - API 调用失败 → 不发邮件，进程以非零码退出（Actions 上会标红，便于发现问题）
"""
...
def send_email(subject: str, text: str) -> None:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            # Resend API 在 Cloudflare 后面，会拦截 Python 默认 UA（403 error 1010），
            # 必须伪装成浏览器 UA 请求才能到达 Resend 本体
            "User-Agent": (
                "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 "
                "(KHTML, like Gecko) Chrome/153.0.0.0 Safari/537.36"
            ),
...
def main() -> int:
    ...
    # 只要有积分房就发邮件（不做状态去重，每次运行都会发）
    available_hotels = {c: i for c, i in results.items() if i["available"]}
    if available_hotels:
        lines = []
        subjects = []
        for code, info in available_hotels.items():
            subjects.append(f"{code} 有积分房")
